...
def main():
    ...
    print("Loading Ollama Instruct model...")
    llm = pipeline("text-generation", model=LOCAL_MODEL_PATH, device_map="auto")

    # Prepare to save results
    analysis_results = {}
    output_file = "C:/Users/ierem/PycharmProjects/pythonProject/analysis_results.json"

    total_threads = len(transcripts)
    print(f"Total conversation threads to analyze: {total_threads}")
    ...
    for idx, (thread_id, transcript) in enumerate(transcripts.items(), start=1):
        ...
        print(f"Processed thread {idx}/{total_threads}")

        # Save intermediate results every 100 threads
        if idx % 100 == 0:
